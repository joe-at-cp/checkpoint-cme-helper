...
then
  ...
  mgmt_cli --session-id $SID set-generic-object uid $GW_UID identityAwareBlade.iaMaxEnforcedIdentities 200000 identityAwareBlade.iaMaxAuthenticatedUsers 200000 -f json

  echo "[ $(date +"%T") $GW_NAME ] - Setting Content Awareness Blade"  >> $LOG
  mgmt_cli --session-id $SID set simple-gateway name $GW_NAME content-awareness true -f

  echo "[ $(date +"%T") $GW_NAME ] - Publishing Session"  >> $LOG
  mgmt_cli publish --session-id $SID -f json
  ...
  mgmt_cli --session-id $SID install-policy targets $GW_NAME policy-package $PACKAGE -f json
